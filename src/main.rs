...
struct Args {
    ...
    #[arg(short, long, env = "GITHUB_WORKSPACE", hide_env_values = true, required = true)]
    workspace: String,

    /// The git base ref to compare against
    #[arg(long, default_value = "HEAD", env = "GITHUB_BASE_REF", hide_env_values = true, )]
    base: String,
...
// fn collect_repo_changes(repo: Repo) -> Result<(), anyhow::Error> {
fn collect_repo_changes(repo: Repo) -> String {
